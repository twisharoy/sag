		...
		<TD class="evenrow-l">
			<table id="responseMapTable" name="responseMapTable" class="tableView" width="100%">
				...
				%else%
					...
					%ifvar responseToMap%
						%loop responseToMap%
							<script>editResponseMapRow('%value ../../../action%','%value address%','%value messageAddressingEndpointAlias%','%value transportType%');</script>
						%endloop%
					%else%
						<script>editResponseMapRow('%value ../../../action%','','','%value transportType%');</script>
					%endif%
					%ifvar action equals('edit')%
