			...
			<td class="oddrow">Use JSSE</td>
		    	...
		    	%ifvar mode equals('view')%
		        	%switch useJSSE%
		             ...
		             %endswitch%
		         %else%
				<input type="radio" name="useJSSE" value="yes" %ifvar useJSSE equals(true)%checked%endif%>Yes</input>
				<input type="radio" name="useJSSE" value="no"  %ifvar useJSSE equals('false')%checked %else% %ifvar ../mode equals('edit')%%else%checked%endif%%endif%>No</input>		         
		          %endif%
		       </td>
	</tr>
	%endif%
