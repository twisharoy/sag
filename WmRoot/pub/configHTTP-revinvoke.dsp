          ...
          if(document.properties1.listening.value=='true'||document.properties2.listening.value=='true')
          {
            if(confirm("Ports must be disabled so that you can edit these settings.  Would you like to disable these ports?"))
            {
				if(is_csrf_guard_enabled && needToInsertToken) {
					document.location.replace("configHTTP.dsp?listenerKey=%value -code listenerKey%&listenerKey1="+document.properties1.listenerKey.value+"&listenerKey2="+document.properties2.listenerKey.value+"&pkg=%value pkg%%ifvar listenerType%&listenerType=%value -code listenerType%%endif%&mode=edit&disableport=true&"+_csrfTokenNm_+"="+_csrfTokenVal_);
				} else { 
					document.location.replace("configHTTP.dsp?listenerKey=%value -code listenerKey%&listenerKey1="+document.properties1.listenerKey.value+"&listenerKey2="+document.properties2.listenerKey.value+"&pkg=%value pkg%%ifvar listenerType%&listenerType=%value -code listenerType%%endif%&mode=edit&disableport=true");
				}
            }
          }
          else {
				if(is_csrf_guard_enabled && needToInsertToken) {	
					document.location.replace("configHTTP.dsp?listenerKey=%value -code listenerKey%&pkg=%value pkg%%ifvar listenerType%&listenerType=%value -code listenerType%%endif%&mode=edit&"+_csrfTokenNm_+"="+_csrfTokenVal_);
				} else {
					document.location.replace("configHTTP.dsp?listenerKey=%value -code listenerKey%&pkg=%value pkg%%ifvar listenerType%&listenerType=%value -code listenerType%%endif%&mode=edit");
				}
		  }	
          return false;
        }
